...
class api_model extends CI_Model {
	...
	function get_ka_akt_list($dimension2_)
	{
	$org_hrd = $this->get_hrd();
	$this->db->distinct();
	$this->db->select("EMPLOYEETABLE.EMPLID,EMPLOYEETABLE.NAME,EMPLOYEETABLE.DIMENSION2_ AS DIMENSION2_");
	$this->db->from("HRSEMPLOYEETABLE AS EMPLOYEETABLE");
	$this->db->join('HRSVIRTUALNETWORKTABLE AS VNTABLE', 'VNTABLE.REFERENCE = EMPLOYEETABLE.EMPLID and EMPLOYEETABLE.DATAAREAID=VNTABLE.DATAAREAID', 'left');
	$this->db->join('HRSVIRTUALNETWORKHISTORY AS VNHISTORYTABLE', 'VNHISTORYTABLE.HRSVIRTUALNETWORKID = VNTABLE.HRSVIRTUALNETWORKID and VNHISTORYTABLE.DATAAREAID=VNTABLE.DATAAREAID','left');
	$this->db->join('HRSPOSITION AS POSITIONTABLE', 'POSITIONTABLE.HRSPOSITIONID = VNHISTORYTABLE.HRSPOSITIONID and POSITIONTABLE.DATAAREAID=VNHISTORYTABLE.DATAAREAID','left');
	...
	$this->db->where('EMPLOYEETABLE.DATAAREAID', 'erl');
	$this->db->where('EMPLOYEETABLE.DIMENSION', $dimension2_);
	$this->db->where('EMPLOYEETABLE.STATUS !=', 2);
	$this->db->where('EMPLOYEETABLE.HRSACTIVEINACTIVE !=', 1);
	$this->db->like('POSITIONTABLE.DESCRIPTION', 'AKUNTING');
	$this->db->like('ORGANIZATIONTABLE.DESCRIPTION', 'AKUNTING');
	$this->db->like('POSITIONTABLE.HRSPOSITIONGROUPID', 'KADEP');
	$this->db->where('BU.DIMENSIONCODE',1);
	$this->db->where('COSTCENTER.DIMENSIONCODE',1);

	/*$query = $this->db->query("SELECT DISTINCT EMPLOYEETABLE.EMPLID, EMPLOYEETABLE.NAME, EMPLOYEETABLE.DIMENSION2_ AS DIMENSION2_ FROM HRSEMPLOYEETABLE AS EMPLOYEETABLE LEFT JOIN HRSVIRTUALNETWORKTABLE AS VNTABLE ON VNTABLE.REFERENCE = EMPLOYEETABLE.EMPLID and EMPLOYEETABLE.DATAAREAID=VNTABLE.DATAAREAID LEFT JOIN HRSVIRTUALNETWORKHISTORY AS VNHISTORYTABLE ON VNHISTORYTABLE.HRSVIRTUALNETWORKID = VNTABLE.HRSVIRTUALNETWORKID and VNHISTORYTABLE.DATAAREAID=VNTABLE.DATAAREAID LEFT JOIN HRSPOSITION AS POSITIONTABLE ON POSITIONTABLE.HRSPOSITIONID = VNHISTORYTABLE.HRSPOSITIONID and POSITIONTABLE.DATAAREAID=VNHISTORYTABLE.DATAAREAID LEFT JOIN HRSORGANIZATION AS ORGANIZATIONTABLE ON ORGANIZATIONTABLE.HRSORGANIZATIONID = VNHISTORYTABLE.HRSORGANIZATIONID and ORGANIZATIONTABLE.DATAAREAID=VNHISTORYTABLE.DATAAREAID LEFT JOIN DIMENSIONS as BU ON BU.NUM = EMPLOYEETABLE.DIMENSION and EMPLOYEETABLE.DATAAREAID=BU.DATAAREAID LEFT JOIN DIMENSIONS AS COSTCENTER ON COSTCENTER.NUM = EMPLOYEETABLE.DIMENSION2_ and EMPLOYEETABLE.DATAAREAID=COSTCENTER.DATAAREAID WHERE EMPLOYEETABLE.DATAAREAID = 'erl' AND EMPLOYEETABLE.DIMENSION = '".$dimension2_."' AND EMPLOYEETABLE.STATUS != 2 AND EMPLOYEETABLE.HRSACTIVEINACTIVE != 1 AND BU.DIMENSIONCODE = 1 AND COSTCENTER.DIMENSIONCODE = 1 AND POSITIONTABLE.DESCRIPTION LIKE '%AKUNTING%' AND ORGANIZATIONTABLE.DESCRIPTION LIKE '%AKUNTING%' AND POSITIONTABLE.HRSPOSITIONGROUPID LIKE '%KADEP%' ");
...
function get_employees_by_position($posid)
{
	$this->db->distinct();
	$this->db->select("EMPLOYEETABLE.EMPLID as EMPLID,EMPLOYEETABLE.NAME AS NAME, POSITIONTABLE.DESCRIPTION AS POSITION");
	$this->db->from("HRSEMPLOYEETABLE AS EMPLOYEETABLE");
...
function get_emp_rekom_by_bu($emplid)
{
	$id_bu = $this->get_empl_bu_id($emplid)['DIMENSION'];
	$id_bu = "'$id_bu'";
	if($id_bu == '50'){
...
function get_user_at_org($orgid)
{
	$parentorgid = $this->get_parent_from_org($orgid)['PARENTORGANIZATIONID'];
	$parent = $this->get_org_from_parent_org($parentorgid);
	$this->db->select('EMPLID, NAME')->FROM('HRSEMPLOYEETABLE')->WHERE('DIMENSION2_', $orgid)->WHERE('HRSEMPLOYEETABLE.DATAAREAID', 'erl')->WHERE('HRSEMPLOYEETABLE.status !=', 2);
	$this->db->where('DIMENSION2_', $orgid);
	//foreach ($parent as $key => $value) {
	//	$this->db->or_where('DIMENSION2_', $value['ID']);
	//}
	$q = $this->db->get(); 
...
function get_user_orgid($emplid)
{
	$null_date_vnhistorytable = (!empty($this->get_null_enddate_vnhistorytable($emplid)['ENDDATE']))?$this->get_null_enddate_vnhistorytable($emplid)['ENDDATE']:'';
	if(!empty($null_date_vnhistorytable)){
		$null_date_vnhistorytable = $this->get_null_enddate_vnhistorytable($emplid)['ENDDATE'];
	...
	$this->db->distinct();
	$this->db->select("ORGANIZATIONTABLE2.DESCRIPTION AS DESCRIPTION, ORGANIZATIONTABLE2.HRSORGANIZATIONID as ID");
	$this->db->from("HRSEMPLOYEETABLE AS EMPLOYEETABLE2");
...
{
	$this->db->distinct();
	$this->db->select('POSITION.HRSPOSITIONID AS ID, POSITION.DESCRIPTION AS DESCRIPTION, POSITION.HRSPOSITIONGROUPID AS POSITIONGROUP, HRSPOSITIONTYPEID AS TYPE, VNHISTORYTABLE2.HRSGRADEID AS GRADE');
	//$this->db->select('POSITION.*');
	
	$this->db->from('HRSPOSITION AS POSITION');
	$this->db->join('HRSVIRTUALNETWORKHISTORY AS VNHISTORYTABLE2', 'POSITION.HRSPOSITIONID = VNHISTORYTABLE2.HRSPOSITIONID and VNHISTORYTABLE2.DATAAREAID=POSITION.DATAAREAID','left');

	$this->db->where('POSITION.HRSORGANIZATIONID', $orgid);
	$this->db->where('POSITION.DATAAREAID', 'erl');

	$this->db->order_by('VNHISTORYTABLE2.HRSGRADEID','desc');
